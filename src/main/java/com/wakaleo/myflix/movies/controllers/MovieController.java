...
public class MovieController {
    ...
    @RequestMapping(value = "/findByTitle/{title}", method = GET)
    @ApiOperation(value = "Find movies from a given title",
            httpMethod = "GET",
            response = Movie.class,
            responseContainer = "List")
    public Movie findByTitle(@PathVariable String title) {
        return repository.findByTitle(capitalizeFully(title.trim()));
    }

    @RequestMapping(value = "/findByActor/{actor}", method = GET)
    @ApiOperation(value = "Find movies staring a given actor",
            httpMethod = "GET",
            response = Movie.class,
            responseContainer = "List")
    public List<Movie> findByActor(@PathVariable String actor) {
        return repository.findByActors(capitalizeFully(actor.trim()));
    }
